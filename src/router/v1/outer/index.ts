...
class Outer extends V1 {
    ...
    async onPostApply(req: Request, res: Response) {
        ...
        const insertApply = await execute(
            "INSERT INTO outer(teacherUid, masterUid, location, purpose, `when`, isApproved) VALUE(?, ?, ?, ?, ?, ?)",
            [
                request.teacherUid,
                payload.uid,
                request.location,
    ...
    async onGetOuterStudentList(req: Request, res: Response) {
        ...
        const information = await query(
            "SELECT B.name, B.applyId, outer.isApproved FROM (SELECT A.name, specialroom_apply_student.applyId FROM (SELECT name, uid FROM user) AS A, specialroom_apply_student WHERE A.uid = specialroom_apply_student.studentUid) AS B, specialroom_apply WHERE B.applyId=specialroom_apply.applyId AND specialroom_apply.when=?",
            [request.when]
        );
